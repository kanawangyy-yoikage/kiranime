...
export const searchNovels = async (query: string, limit = 20) => {
...
export const getNovelDetail = async (url: string): Promise<NovelDetail> => {
...
export const getNovelEpisodes = async (url: string, page = 1) => {
  const sep = url.includes('?') ? '&' : '?'
  const html = await fetchHtml(`${url}${sep}page=${page}`)
  const $ = cheerio.load(html)
  const list: NovelEpisode[] = []
...
export const getTrendingNovels = async (day = 'trending') => {
