...
export default function GroupDetailPage() {
  ...
  if (!group) {
    return (
      <div className="card p-10 text-center space-y-3">
        <p className="text-pearl/60">Grup tidak ditemukan atau kamu sudah tidak menjadi anggota.</p>
  ...
  if (!isMember) {
    return (
      <div className="card p-10 text-center space-y-3">
        <p className="text-pearl/60">Kamu bukan anggota grup ini.</p>
        <Link href="/groups" className="btn-secondary inline-flex items-center gap-2">
  ...
  return (
    <>
      <Head><title>{group.name} - Grup KiraStream</title></Head>
      <div className="space-y-5">
        <div className="flex items-center gap-3">
          <Link href="/groups" className="btn-secondary p-2" aria-label="Kembali">
            <ArrowLeft size={18} />
          </Link>
        ...
        <div className="card p-4">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-pearl/80 flex items-center gap-2">
              <Users size={16} className="text-ocean" /> Anggota ({members.length})
            </h2>
            ...
            {members.map((m) => (
              <div key={m.uid} className="flex items-center gap-2 p-2 rounded-xl bg-surface-dark">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {m.uid === group.ownerId && <p className="text-[10px] text-ocean">Pemilik</p>}
                </div>
                {isOwner && m.uid !== user?.uid && (
                  <button
                    onClick={() => handleKick(m.uid, m.displayName)}
                    className="p-1 rounded hover:bg-red-500/10 text-red-400"
                    aria-label={`Keluarkan ${m.displayName}`}
                  >
                    <UserMinus size={13} />
                  </button>
                )}
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-bold text-lg text-pearl flex items-center gap-2">
                <Users size={18} className="text-ocean" /> Undang Teman
              </h2>
              <button onClick={() => setInviteOpen(false)} className="p-2 rounded-lg hover:bg-pearl/10" aria-label="Tutup">
                <X size={18} />
              </button>
            </div>
            ...
            >
              {saving ? <Loader2 size={16} className="animate-spin" /> : 'Undang'}
            </button>
          </div>
        </>
      )}
